"""
This file contains code for the game "Gemini CLI Board Game".
...
class SavedGameData:
    ...
    def __init__(self, player_name, temperature, top_p, top_k, max_output_tokens, start_bonus, player_data, ai_player, board):
        # type: (str, float, float, float, int, mpf, Player, AIPlayer, Board) -> None
...
def main() -> int:
    ...
    # Saved game data
    saved_game_data: SavedGameData = SavedGameData("", 1, 0.95, 64, 8192,
                                    mpf(random.randint(100000, 500000)), Player(""), AIPlayer(),
                                    Board([]))  # initial value
    ...
    while not game_started:
        if action == "NEW GAME":
            ...
            saved_game_data = SavedGameData(player_name, 1, 0.95, 64, 8192,
                                            mpf(random.randint(100000, 500000)), Player(player_name), AIPlayer(),
                                            board)
            game_started = True
        else:
            clear()
